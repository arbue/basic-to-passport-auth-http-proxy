...
import { LoginSessionStore } from './LoginSessionStore.js';
import { PassportSession } from './PassportSession.js';
import { PassportHandler } from './PassportHandler.js';
import { PassportError } from './PassportError.js';
import { RequestBodyCache } from './RequestBodyCache.js';
import { proxyConfig } from './proxyConfig.js';
import { handleResponseWithHttpProxy } from './handleResponseWithHttpProxy.js';
...
export class Server {
    ...
    constructor() {
        this.proxy = ProxyServer.createProxyServer(proxyConfig);
        this.server = createServer((req, res) => this.proxy.web(req, res));
         ...
         */
        this.proxy.before('web', 'stream', (req, res, options) => {
        ...
        this.proxy.on('proxyRes', async (proxyRes, req, res) => {
            ...
            if (PassportHandler.isPartnerServerChallengeMessage(proxyRes.statusCode, authenticateHeader)) {
                ...
                if (!username || !password) {
                    this.sendResponse(proxyRes, req, res, 401, 'Username and/or password missing.');
                ...
                try {
                    ...
                    if (error instanceof PassportError && (error.statusCode === 401 || error.statusCode === 403)) {
                        this.sendResponse(proxyRes, req, res, error.statusCode, error.message, error.body);
                    } else {
                        log.e(error.message);
                        this.sendResponse(proxyRes, req, res, 500, error.message);
                    }
            ...
            this.sendResponse(proxyRes, req, res);
        });
        ...
        this.proxy.on('error', (err, req, res) => {
            this.sendResponse(null, req, res, 500, err.message || '' + err);
        });


        this.server.on('listening', () => {
            log('Proxy server listening: %o', this.server.address());
        });
     ...
     */
    start() {
        this.server.listen(process.env.PROXY_PORT || 3000);
    }
     ...
     */
    sendResponse(proxyRes, req, res, statusCode, errorMessage, body) {
        if (statusCode) {
        ...
        handleResponseWithHttpProxy(proxyRes, req, res, this.proxy.options);
    }
